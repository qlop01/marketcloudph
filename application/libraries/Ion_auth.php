...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ci->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			$this->ci->email->from($this->ci->config->item('admin_email', 'ion_auth'), $this->ci->config->item('from_title', 'ion_auth'));
			$this->ci->email->to($user->email);
			$this->ci->email->subject($this->ci->config->item('email_title', 'ion_auth') . ' - Forgotten Password Verification');
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			$this->ci->email->subject($this->ci->config->item('email_title', 'ion_auth') . ' - New Password');
			$this->ci->email->message($message);
	...
	public function register($username, $password, $email, $additional_data, $group_name = false) //need to test email activation
	{
		...
		else
		{
			...
			$this->ci->email->subject($this->ci->config->item('email_title', 'ion_auth') . ' - Account Activation');
			$this->ci->email->message($message);
